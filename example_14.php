...
if ((isset($_POST['submit7']))||(isset($_POST[submit8]))){
$uploaddir='./uploads/';
$file = $_FILES['uploadfile4']['tmp_name'];
...
if(!empty($file))
{
  ...
  $size = filesize ($_FILES['uploadfile4']['tmp_name']);
...
{
     pg_query($dbconn,"update dp01 set pic4='".$new_name."' where id_data='".$_POST['idstrr4']."';");}   
 else if (isset($_POST['submit8']))
{
    $f=pg_query($dbconn,"insert into pictureprob04 (pict) values ('".$new_name."') RETURNING id_pic;");
    $ff=pg_fetch_all($f);
    foreach ($ff as $ll=>$ii)
    {
     $y=pg_query($dbconn,"select pict from pictureprob04 where id_pic='".$ii['id_pic']."';");
     $i=pg_fetch_all($y);
     foreach ($i as $pp=>$ll)
     {
      pg_query($dbconn,"update dp01 set pic4='".$ll['pict']."' where id_data='".$_POST['idstrr4']."';");
     }
}
}
      }
